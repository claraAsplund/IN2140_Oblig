...
void writeOut(char* username, char* countryCode, char* postNumber, char* writeBuffer) {
    ...
    // Her kan det være nyttig å bruke malloc

    char* writeOutBuffer = malloc(writeSize);
    ...
    for(int i = 0; i < strlen(username); i++) {
        ...
        writeOutBuffer[writeStart++] = username[i];

    }

    for(int i = 0; i < strlen(countryCode); i++) {

        writeOutBuffer[writeStart++] = countryCode[i];
    ...
    for(int i = 0; i < strlen(postNumber); i++) {
        ...
        writeOutBuffer[writeStart++] = postNumber[i];

    }


    writeOutBuffer[writeStart] = '\0';

    printf("String is %s", writeOutBuffer);
